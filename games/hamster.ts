...
interface AccountResults {
  Account: null | string;
  User: null | string;
  BalanceBefore: number | string | undefined;
  BalanceAfter: number | string | undefined;
  ProfitPerHour: number | string | undefined;
}
...
async function repeatCheckAndReload(page: Page, maxAttempts: number) {
  let reloadCount = 0;
...
async function processMineItems(page: Page, initialBalance: number) {
  ...
  while (true) {
    ...
    let currentTotal = 0;
    const cardsToClick: { element: ElementHandle<Element>; value: number }[] = [];
...
async function performClick(page: Page, x: number, y: number, duration: number) {
  await page.mouse.move(x, y);
  await page.mouse.down();
